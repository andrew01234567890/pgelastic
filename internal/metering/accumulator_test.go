...
func TestStatsResetIsARestartEvenWhenTheCountersRose(t *testing.T) {
	...
	if total := accumulator.Total(key, StatXactCommit); total != 240 {
		t.Errorf("total = %d, want 240: a new stats_reset means the 140 is counting from a new origin, "+
			"so the delta is 140 and not 40", total)
	}
}
...
func TestPrimaryAndReplicaCountersAreNeverDifferencedAgainstEachOther(t *testing.T) {
	...
	if total := accumulator.Total(replica, StatXactCommit); total != 12 {
		t.Errorf("replica total = %d, want 12", total)
	}
	if total := accumulator.Total(primary, StatXactCommit); total != 900 {
		t.Errorf("primary total = %d, want 900", total)
	}
}
...
func TestADatabaseRecreatedUnderTheSameNameRestartsItsCount(t *testing.T) {
	...
	if total := accumulator.Total(key, StatXactCommit); total != 409 {
		t.Errorf("total = %d, want 409: a new dbid is a new counter sequence, not a decrease", total)
	}
}
