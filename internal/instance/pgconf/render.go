...
func RenderHBA(config HBAConfig) string {
	...
	// The ops role's socket rule has to precede the peer catch-all below it, and the whole
	// value of the line is that placement. pg_hba matches the first record that fits, and the
	// agent runs as the OS user postgres, so under "local all all peer" a connection asking
	// for pgelastic_ops is checked against the peer identity postgres, does not match, and is
	// rejected. Below the catch-all this rule is unreachable and the scrape can only ever run
	// as the bootstrap superuser - which is precisely the identity CVE-2026-44477 made worth
	// keeping off a metrics path.
	if config.OpsRole != "" {
		fmt.Fprintf(&builder, "local all %s scram-sha-256\n", config.OpsRole)
	}
